...
<body class="ig-body antialiased min-h-screen">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div class="relative" id="notif-wrapper">
                        ...
                        <div id="notif-dropdown" class="hidden absolute right-0 mt-2 w-80 bg-white rounded-2xl border border-[var(--ig-line)] shadow-[var(--shadow-lg)] z-50 overflow-hidden">
                            ...
                            <div class="px-4 py-3 border-t border-[var(--ig-line)] text-center bg-[var(--ig-bg)]">
                                <a href="{{ route('notifications.index') }}" class="text-[12px] font-semibold text-[var(--ig-ink)] hover:text-[var(--ig-accent)]">View all →</a>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="relative group">
                        <button class="flex items-center gap-2.5 pl-2 pr-3.5 py-1.5 rounded-full border border-[var(--ig-line)] hover:border-[var(--ig-ink)] transition">
                            <span class="ig-avatar text-[12px]" style="width:28px;height:28px;">{{ strtoupper(substr(auth()->user()->name,0,1)) }}</span>
                            <span class="text-[13px] font-medium">{{ Str::limit(auth()->user()->name, 14) }}</span>
    ...
    <!-- ───────── FLASH MESSAGES ───────── -->
    @foreach (['success' => ['msg' => session('success'), 'cls' => 'ig-banner-success'],
               'message_sent' => ['msg' => session('message_sent'), 'cls' => 'ig-banner-success'],
               'task_created' => ['msg' => session('task_created'), 'cls' => 'ig-banner'],
               'error' => ['msg' => session('error'), 'cls' => 'ig-banner-warn']] as $key => $f)
        @if($f['msg'])
            <div id="flash-{{ $key }}" class="ig-container mt-6 ig-anim-fade-up">
                <div class="ig-banner {{ $f['cls'] }} ig-z10">
                    ...
                    <p class="text-sm font-medium flex-1">{{ $f['msg'] }}</p>
                    <button onclick="document.getElementById('flash-{{ $key }}').remove()" class="text-[var(--ig-muted)] hover:text-[var(--ig-ink)]">
