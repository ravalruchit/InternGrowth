                ...
                <div class="flex items-center space-x-6">
                    ...
                    <div>
                        <div class="flex flex-wrap items-center gap-3">
                            <h1 class="text-3xl font-bold tracking-tight font-poppins">{{ $profile->company_name }}</h1>
                        ...
                        <p class="text-slate-400 text-sm mt-2 flex items-center gap-2">
                            ...
                            <span>{{ $profile->company_address ?? 'Address not updated' }}</span>
                        </p>
                    </div>
                </div>
            ...
            <div class="bg-gradient-to-br from-indigo-600 via-indigo-700 to-purple-800 rounded-2xl shadow-xl p-6 text-white border border-indigo-500/20 transform hover:-translate-y-1 transition duration-300">
                <div class="flex justify-between items-center">
                    <div>
                        ...
                        <p class="text-4xl font-extrabold tracking-tight mt-2">{{ number_format($overall, 0) }}/100</p>
            ...
            <div class="bg-white rounded-2xl shadow-md p-6 border border-gray-150 transform hover:-translate-y-1 transition duration-300">
                <div class="flex justify-between items-center">
                    <div>
                        ...
                        <p class="text-4xl font-extrabold text-slate-900 tracking-tight mt-2">{{ $studentsHired }}</p>
            ...
            <div class="bg-white rounded-2xl shadow-md p-6 border border-gray-150 transform hover:-translate-y-1 transition duration-300">
                <div class="flex justify-between items-center">
                    <div>
                        ...
                        <p class="text-4xl font-extrabold text-slate-900 tracking-tight mt-2">{{ number_format($paymentScoreVal, 0) }}%</p>
                        <p class="text-[10px] text-green-600 font-medium mt-2">
                            {{ $paymentScoreVal >= 100 ? '✅ 100% Reliable' : '⚠️ Balance Deficit warning' }}
                        </p>
                    </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2 space-y-8">
                ...
                <div class="bg-white rounded-3xl shadow-lg p-8 border border-gray-150">
                    ...
                    <p class="text-gray-650 leading-relaxed text-sm whitespace-pre-line">{{ $profile->description ?? 'No company description provided.' }}</p>
                </div>
                            ...
                            <div class="border border-gray-150 hover:border-indigo-250 hover:shadow-xl rounded-2xl p-5 transition duration-300 group">
                                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                                    <div>
                                        ...
                                        <p class="text-xs text-gray-500 mt-1">Reward Points: <span class="font-bold text-indigo-600">{{ $task->reward_points }} pts</span></p>
                                        <div class="flex flex-wrap gap-1.5 mt-3">
                                            @foreach($task->skills as $skill)
                                                <span class="text-[10px] bg-slate-100 text-slate-700 px-2 py-0.5 rounded font-medium">{{ $skill->name }}</span>
                                            @endforeach
                                        </div>
                                    </div>
                                    <div class="flex sm:flex-col items-start sm:items-end justify-between sm:justify-center gap-2">
                                        @if($task->stipend)
                                            <span class="text-lg font-extrabold text-green-600 font-poppins">₹{{ number_format($task->stipend, 0) }}</span>
                                        @endif
                    ...
                    <div class="space-y-5">
                        ...
                        <div>
                            <div class="flex justify-between text-xs font-semibold text-gray-500 mb-1.5">
                                ...
                                <span class="font-bold text-gray-900">{{ number_format($verifyScore, 0) }}%</span>
                            </div>
                            <div class="w-full bg-gray-100 rounded-full h-2">
                                <div class="bg-indigo-500 h-2 rounded-full transition-all duration-500" style="width: {{ $verifyScore }}%"></div>
                            </div>
                        </div>
                        ...
                        <div>
                            <div class="flex justify-between text-xs font-semibold text-gray-500 mb-1.5">
                                ...
                                <span class="font-bold text-gray-900">{{ number_format($paymentScore, 0) }}%</span>
                            </div>
                            <div class="w-full bg-gray-100 rounded-full h-2">
                                <div class="bg-emerald-500 h-2 rounded-full transition-all duration-500" style="width: {{ $paymentScore }}%"></div>
                            </div>
                        </div>
                        ...
                        <div>
                            <div class="flex justify-between text-xs font-semibold text-gray-500 mb-1.5">
                                ...
                                <span class="font-bold text-gray-900">{{ number_format($ratingScore, 0) }}%</span>
                            </div>
                            <div class="w-full bg-gray-100 rounded-full h-2">
                                <div class="bg-amber-500 h-2 rounded-full transition-all duration-500" style="width: {{ $ratingScore }}%"></div>
                            </div>
                        </div>
                        ...
                        <div>
                            <div class="flex justify-between text-xs font-semibold text-gray-500 mb-1.5">
                                ...
                                <span class="font-bold text-gray-900">{{ number_format($hiringScore, 0) }}%</span>
                            </div>
                            <div class="w-full bg-gray-100 rounded-full h-2">
                                <div class="bg-purple-500 h-2 rounded-full transition-all duration-500" style="width: {{ $hiringScore }}%"></div>
                            </div>
                        </div>
                        ...
                        <div>
                            <div class="flex justify-between text-xs font-semibold text-gray-500 mb-1.5">
                                ...
                                <span class="font-bold text-gray-900">{{ number_format($taskScoreVal, 0) }}%</span>
                            </div>
                            <div class="w-full bg-gray-100 rounded-full h-2">
                                <div class="bg-pink-500 h-2 rounded-full transition-all duration-500" style="width: {{ $taskScoreVal }}%"></div>
                            </div>
                        </div>
                    </div>
                </div>
